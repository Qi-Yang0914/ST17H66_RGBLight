...
static void hal_init(void)
{
    ...
    LOG_INIT();
    hal_gpio_init();				
	hal_fs_init(0x1103C000,2);//	注意：0x1103e000起始的4K空间有存放频偏校准数据，初始化FS不能使用第3个4K空间，只能使用0x1103c000 -- 0x1103dfff这两个4K空间；
}


/////////////////////////////////////////////////////////////////////////////////////////////////////////
extern	void	rf_phy_dtm_ext_rx_demod_burst(uint8 rfChnIdx, int8 rfFoff, uint8 pktLength, uint32 rxTimeOut, uint32 rxWindow, uint16* rxEstFoff,uint8* rxEstRssi,uint8* rxEstCarrSens, uint16* rxPktNum);
...
static	void	rf_DTM_RX_rfoffset_calibration(void)
{
	...
	hal_flash_read(0x1103e000, rfFreqOffset_flash, 64);
		...
		while(1){
			if(rfFreqOffset_flash[FLASH_RfFreOffset_ADD] != 0x55){
				...
				if(rxPktNum != 0){
					...
					hal_flash_erase_sector(0x1103e000);
					WaitMs(10);
					hal_flash_write(0x1103e000, rfFreqOffset_flash, 64);
					hal_gpio_fmux(P10, Bit_DISABLE);
					hal_gpio_pin_init(P10, OEN);
					hal_gpio_write(P10, 0);
					hal_flash_read(0x1103e000 + FLASH_RfFreOffset_ADD, rfFreqOffset_flash + FLASH_RfFreOffset_ADD, 1);
				}
			}else{
				hal_gpio_fmux(P10, Bit_DISABLE);
				hal_gpio_pin_init(P10, OEN);
