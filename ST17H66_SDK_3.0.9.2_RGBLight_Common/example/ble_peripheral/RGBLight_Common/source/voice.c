...
voice_Ctx_t	mVoiceCtx;
uint32_t	voice_data[HALF_VOICE_WORD_SIZE];
...
int hal_voice_config(voice_Cfg_t cfg, voice_Hdl_t evt_handler)
{
  ...
  if(cfg.voiceSelAmicDmic) {
		...
		hal_gpio_pull_set(P18, GPIO_FLOATING);
		hal_gpio_pull_set(P20, GPIO_FLOATING);
...
int hal_voice_start(void)
{
  ...
  NVIC_SetPriority((IRQn_Type)ADCC_IRQn, IRQ_PRIO_THREAD);//teddy add 20190121	
